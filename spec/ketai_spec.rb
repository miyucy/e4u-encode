...
describe E4U::Encode::DoCoMo do
  ...
  describe "encode" do
    ...
    it "from[:carrier]が:ketai以外ならば例外が起こる" do
      @from.update :carrier => :not_ketai
      lambda {
        E4U::Encode::Ketai.encode('', @from, @to)
      }.should raise_error
    end

    context "to[:carrier]が:ketai以外の時" do
      before :all do
        @to.update :carrier => :not_ketai
      end

      before :each do
        @from.update :carrier => :ketai
      end
      ...
      it "from[:encoding]が:unicodeなら from[:carrier]が:googleに変換される" do
        ...
        lambda {
          E4U::Encode::Ketai.encode('', @from, @to)
        }.should change { @from[:carrier] }.from(:ketai).to(:google)
      end
    end

    context "to[:carrier]が:ketaiの時" do
      before :all do
        @to.update :carrier => :ketai
      end
      ...
      context "from[:encoding]が:unicodeの時" do
        ...
        it "to[:encoding]が:sjisならば、CP932な文字列が返ってくる" do
          ...
          @to.update :encoding => :sjis
          E4U::Encode::Ketai.encode(str, @from, @to).encoding.should == Encoding::Windows_31J
          # うまく動かない…
          # lambda {
          #   E4U::Encode::Ketai.encode(str, @from, @to)
        ...
        it "to[:encoding]が:utf8ならば、UTF-8な文字列が返ってくる" do
          ...
          str = "とある文字列の符号化方式＜エンコーディング＞"
          @to.update :encoding => :utf8
          E4U::Encode::Ketai.encode(str, @from, @to).encoding.should == Encoding::UTF_8
          # lambda {
          #   E4U::Encode::Ketai.encode(str, @from, @to)
